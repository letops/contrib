# Obtained and edited from https://djangosnippets.org/snippets/863/
...
class ChoiceWithOtherRenderer(forms.RadioSelect.renderer):
    ...
    def __iter__(self):
        for input in super(ChoiceWithOtherRenderer, self).__iter__():
            yield input
        id = '%s_%s' % (self.attrs['id'], self.other[0]) if \
            'id' in self.attrs else ''
        label_for = ' for="%s"' % id if id else ''
        checked = '' if not encoding.force_unicode(self.other[0]) == self.value \
            else 'checked="true" '
...
class ChoiceWithOtherField(forms.MultiValueField):
    ...
    #
    # The last item in the choices array passed to __init__ is expected to be
    # a choice for "other". This field's cleaned data is a tuple consisting of
    # the choice the user made, and the "other" field typed in if the choice
    ...
    # ...     ])
    # ...
    # >>> # rendered as a RadioSelect choice field whose last choice has a
    # >>> #  text input
    # ... print AgeForm()['age']
    # <ul>
    # <li><label for="id_age_0_0"><input type="radio" id="id_age_0_0"
    #   value="0" name="age_0" /> 15-29</label></li>
    # <li><label for="id_age_0_1"><input type="radio" id="id_age_0_1"
    #   value="1" name="age_0" /> 30-44</label></li>
    # <li><label for="id_age_0_2"><input type="radio" id="id_age_0_2"
    #   value="2" name="age_0" /> 45-60</label></li>
    # <li><label for="id_age_0_3"><input type="radio" id="id_age_0_3"
    #   value="3" name="age_0" /> Other, please \
    ...
    # >>> form = AgeForm({'age_0': 1, 'age_1': 'This is bogus text which is
    #   ignored since I didn't pick "other"'})
    # >>> form.is_valid()
    # True
    # >>> form.cleaned_data
    # {'age': (u'1', u'')}
    ...
    def compress(self, value):
        ...
        if not value:
            return [None, u'']
        return (value[0], value[1] if encoding.force_unicode(value[0]) == encoding.force_unicode(
            self.fields[0].choices[-1][0]) else u''
        )
